...
def minus_month(seq):
    prev_month = timedelta(days=31)
    new_seq = []
    for x in seq:
        new_date = x - prev_month
        floored_date = date(new_date.year, new_date.month, 1)
        new_seq.append(floored_date)
    return new_seq
...
# month to month
def get_month_prev(percent_change_file, percent=6):
    econ_seq = load_percent_change(percent_change_file)
    sorted_econ = sorted(econ_seq, key=econ_seq.get)
    ...
    # take month of downturn as "bad" and previous  month as "good"
    return minus_month(prev_bad),prev_bad
