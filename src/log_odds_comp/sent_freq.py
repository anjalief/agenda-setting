...
def ComputeSentPMI(frame_to_lex, articles):
    ...
    return frame_to_sent_and_usa_count, usa_sent_count
    frame_to_pmi = {}
...
def main():
    ...
    good_pmi, c = ComputeSentPMI(frame_to_lex, good_articles)

    for f in good_pmi:
        print(f, "|", good_pmi[f] / float(c))
    ...
    bad_pmi,c = ComputeSentPMI(frame_to_lex, bad_articles)
    for f in bad_pmi:
        print(f, "|", bad_pmi[f] / float(c))
